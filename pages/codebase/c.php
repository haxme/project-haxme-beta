...
<a href="?p=base">CodeBase</a> &raquo; C
</div>
...
if (is_numeric($_GET['id']) == true) {
    ...
    $all = mysql_query("SELECT * FROM `codebase` WHERE `id`='$id' && `lang`='c'") or die(mysql_error());
    ...
    require('sources/interface/db_core.php');
    $all = mysql_query("SELECT * FROM `codebase` WHERE `lang` = 'c' ORDER BY `id`") or die(mysql_error());
    ?>
    <div align="center">
    <table>
    <tr>
    ...
    <?php while ($row = mysql_fetch_array($all)) {
        ...
        <td><div align="center" style="padding-left:8px; padding-top:2px;"><a href="?p=base&lang=c&id=<?php echo $row['id'];
